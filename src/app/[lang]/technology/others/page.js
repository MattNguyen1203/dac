...
} from '@/graphql/technology/query'
import React from 'react'

const PARAM_ARR = [GET_SLUG_OFFSET, GET_SLUG_FLEXO, GET_SLUG_GRAVURE, GET_SLUG_DIGITAL, GET_SLUG_OTHERPRINT]
...
export default async function page({ params }) {
  ...
  const [sluOffset, slugFlexo, slugGravure, slugDigital, slugOther] = await Promise.all(
    PARAM_ARR.map((item) => {
  ...
  const listSlug = {
    slugVi:
      '/cong-nghe/' +
      (listSlugOther?.data?.page?.translations[0]?.language?.code === 'VI'
        ? listSlugOther?.data?.page?.translations[0]?.slug
        : listSlugOther?.data?.page?.slug),
    slugEn:
      '/en/technology/' +
      (listSlugOther?.data?.page?.translations[0]?.language?.code === 'EN'
        ? listSlugOther?.data?.page?.translations[0]?.slug
        : listSlugOther?.data?.page?.slug)
  }
  return (
