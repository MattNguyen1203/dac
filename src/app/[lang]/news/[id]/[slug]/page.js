...
export default async function page({ params: { lang, slug, id } }) {
  ...
  const listSlug = {
    slugVi:
      item?.slugVi +
      '/' +
      (listSlugBlogDetail?.data?.post?.translations[0]?.language?.code === 'VI'
        ? listSlugBlogDetail?.data?.post?.translations[0]?.slug
        : listSlugBlogDetail?.data?.post?.slug),
    slugEn:
      item?.slugEn +
      '/' +
      (listSlugBlogDetail?.data?.post?.translations[0]?.language?.code === 'EN'
        ? listSlugBlogDetail?.data?.post?.translations[0]?.slug
        : listSlugBlogDetail?.data?.post?.slug)
  }
  return (
